...
from tests import pricing  # noqa: E402
...
class HermeticSuiteTests(unittest.TestCase):
    """The suite must not read the developer's shell.

    ONEWORD_PRICE outranks the price table by design, which is right for a run
    and fatal for a test: anyone who had exported a real price got five
    failures that had nothing to do with their code. The helper now states the
    environment instead of inheriting it, and this is what keeps that true.
    """

    def test_an_exported_price_does_not_reach_the_tests(self):
        with mock.patch.dict(os.environ, {"ONEWORD_PRICE": "99.00"}):
            use_test_price(self, price=0.75)
            cost = vendors.estimated_cost_cny(
                config(model=pricing.TEST_MODEL, resolution="720p", duration=5)
            )
        self.assertEqual(cost, 0.75)

    def test_an_exported_price_file_does_not_reach_the_tests_either(self):
        with mock.patch.dict(os.environ, {"ONEWORD_PRICES": "/nowhere/prices.json"}):
            use_test_price(self, price=0.75)
            cost = vendors.estimated_cost_cny(
                config(model=pricing.TEST_MODEL, resolution="720p", duration=5)
            )
