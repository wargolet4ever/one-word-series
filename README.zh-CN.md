...
| `keep` | clip 自带的声音优先；clip 本身没声音时照样配旁白 |
| `replace` | 只要旁白，丢掉 clip 自带音轨 |

自带人声的镜头根本不会触发 TTS 调用，所以不会出现两个声音抢着说同一句。
无声的 clip 在三种模式下表现完全一致 —— 这个模式决定的是「已经存在的那个声音怎么办」，
没有声音的时候它就没什么可决定的。

**Seedance 默认不生成音频**。在有可用配音引擎时，`--audio keep` 会为无声镜头
加入旁白；要让视频模型自己演台词，请设置 `SEEDANCE_AUDIO=1`。
...
2. **用真实素材量出来的阈值。** 现在的阈值仍然来自合成靶场，而第一次真实运行
   ...
   因为两集构不成一个分布。
3. **仓库里放一条完整演示片。** README 顶部已有演示 GIF；付费成片已经有了，但尚未提交完整影片。
