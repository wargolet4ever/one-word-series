...
2. **音乐与环境声。** 现在只有对白轨。
3. **没有模型时的人物漂移。** 场景有本地初筛，人物必须有多模态 key，
   否则如实留空不检查。
4. **用真实素材重新标定阈值。** 现在的阈值标定在合成靶场上。等有了真实的
   Seedance 成片，拿 `scripts/calibrate_drift.py` 对着它重跑一遍再改常数。
5. **真实付费生成的证据。** 适配器写完了、mock 测过了，但还没出过一次付费成片。
