...
from .bible import SeriesBible, build_bible, slugify
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            "rewrite the story and generate every shot again — "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # A series already shot under this word keeps its story. Rewriting it
        # means new prompts, which means nothing on disk matches and every
        # clip is bought again — the resume feature defeated by the first step
        # of the run. Running the same command twice must not cost twice.
        standing = Path(args.out) / slugify(args.word) / "bible.json"
        if not args.bible and not args.fresh and standing.is_file():
            args.bible = str(standing)
            print(f"· a series for \"{args.word}\" already exists; keeping its story")
            print("  (--fresh rewrites it, and then every shot is bought again)")
